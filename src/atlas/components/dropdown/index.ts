...
export function createDropdown(
	...
	// SSR guard
	if (!isBrowser()) {
		return createNoopDropdownState(
			trigger,
			menu,
	...
	const focusItem = (index: number) => {
		...
		menuItems.forEach((item) =>
			item.classList.remove("atlas-dropdown-focused"),
		);
	...
	const open = () => {
		...
		cleanupListeners.push(
			addListener(
				document,
				"click",
				handleClickOutside as (ev: DocumentEventMap["click"]) => void,
			),
			addListener(
				menu,
				"keydown",
				handleMenuKeyDown as (ev: DocumentEventMap["keydown"]) => void,
			),
			addListener(
				menu,
				"click",
				handleItemClick as (ev: DocumentEventMap["click"]) => void,
			),
		);
